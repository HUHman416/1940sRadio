...
class StationStore extends ChangeNotifier {
  ...
  List<RadioStation> get favorites => _stations
  ...
  RadioStation get selectedStation =>
      stationById(_selectedStationId) ?? RadioStation.fogPoint;
  ...
  Future<void> load() async {
    ...
    if (remoteBuiltIns.isNotEmpty) {
      final fogPoint = remoteBuiltIns
          .where((station) => station.id == RadioStation.fogPoint.id)
          .firstOrNull;
      if (fogPoint != null) {
        _stations[0] = fogPoint;
      }
    ...
    if (savedPresets != null && savedPresets.length == 6) {
      _presets =
          savedPresets.map((value) => value.isEmpty ? null : value).toList();
    }
  ...
  Future<void> updateStation(
    ...
    final index =
        _stations.indexWhere((candidate) => candidate.id == station.id);
    if (index < 0) {
      return;
    }
  ...
  Future<void> toggleFavorite(RadioStation station) async {
    if (_favorites.contains(station.id) &&
        station.id != RadioStation.fogPoint.id) {
      _favorites.remove(station.id);
    } else {
  ...
  Future<void> importConfiguration(String raw) async {
    ...
    _stations
      ...
      ..addAll(
        imported.where((station) => station.id != RadioStation.fogPoint.id),
      );
    ...
    _selectedStationId = stationById(selected) == null
        ? RadioStation.fogPoint.id
        : selected!;
